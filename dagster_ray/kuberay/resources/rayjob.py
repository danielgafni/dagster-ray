...
class KubeRayInteractiveJob(BaseKubeRayResourceConfig, BaseRayResource):
    ...
    ray_job: RayJobConfig = Field(
        default_factory=RayJobConfig, description="Configuration for the Kubernetes `RayJob` CR"
    )
    client: KubeRayJobClientResource = Field(
    ...
    def job_name(self) -> str:
        if self._job_name is None and self.rayjob.metadata.get("name") is None:
            raise ValueError(f"{self.__class__.__name__} not initialized")
        elif (name := self.rayjob.metadata.get("name")) is not None:
            return name
        else:
    ...
    def get_dagster_tags(self, context: InitResourceContext | OpOrAssetExecutionContext) -> dict[str, str]:
        ...
        tags.update({"dagster.io/rayjob": self.job_name, "dagster.io/deployment": self.deployment_name})
        return tags
    ...
    def create_and_wait(self, context: InitResourceContext | OpOrAssetExecutionContext):
        ...
        self._job_name = self.rayjob.metadata.get("name") or self._get_step_name(context)
        ...
        try:
            ...
            self.client.wait_until_ready(name=self.job_name, namespace=self.namespace)
            self._cluster_name = self.client.ray_ray_cluster_name(self.job_name, self.namespace)
            self._host = self.client.client.get_status(name=self.cluster_name, namespace=self.namespace)[  # pyright: ignore
                "head"
            ]["serviceIP"]

            msg = f"RayJob {self.namespace}/{self.job_name} has created a read RayCluster {self.namespace}/{self.cluster_name}! Connection command:\n"
