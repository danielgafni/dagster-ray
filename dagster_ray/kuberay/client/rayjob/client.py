...
class RayJobClient(BaseKubeRayClient[RayJobStatus]):
    ...
    def wait_until_running(
        ...
        while True:
            ...
            if status in ["Running", "Complete"]:
                break
            if status == "Failed":
                msg = f"RayJob {namespace}/{name} deployment failed. Status:\n{status}"
            ...
            if time.time() - start_time > timeout:
                ...
                msg = f"Timed out waiting for RayJob {namespace}/{name} to start. Status:\n{status}"
    ...
    def _wait_for_job_submission(
        ...
        while True:
            ...
            if (job_status := status.get("jobStatus")) is not None and job_status != "PENDING":
                return

            if time.time() - start_time > timeout:
                msg = f"Timed out waiting for job {name} to start"
                raise TimeoutError(msg)
    ...
    def get_job_logs(self, name: str, namespace: str, timeout: int = 60 * 60, port_forward: bool = False) -> str:
        ...
        with self.ray_cluster_client.job_submission_client(
            name=self.get_ray_cluster_name(name, namespace),
            namespace=namespace,
