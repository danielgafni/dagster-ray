...
def get_job_submission_id_from_run_id(run_id: str, resume_attempt_number=None) -> str:
...
class RayLauncherConfig(RayExecutionConfig, RayJobSubmissionClientConfig):
    env_vars: list[str] | None = Field(
        ...
        Each can be of the form KEY=VALUE or just KEY (in which case the value will
        be pulled from the current process).""",
    )
...
class RayRunLauncher(RunLauncher, ConfigurableClass):
    ...
    @property
    def supports_run_worker_crash_recovery(self) -> bool:
        return True
    ...
    def _launch_ray_job(self, submission_id: str, entrypoint: str, run: DagsterRun) -> None:
    ...
    def check_run_worker_health(self, run: DagsterRun):
        ...
        if run.status in (DagsterRunStatus.STARTED, DagsterRunStatus.CANCELING) and status.is_terminal():
            return CheckRunHealthResult(
                WorkerStatus.FAILED,
                f"Run has not completed but Ray job has is in status: {status}",
            )
        ...
        if status == JobStatus.STOPPED:
            ...
            return CheckRunHealthResult(
                WorkerStatus.FAILED,
                f"Ray job has been stopped externally. Message: {job_details.message}",
            )
